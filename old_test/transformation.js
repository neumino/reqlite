...
describe('transformation.js.js', function(){
  ...
  it('offsetsOf', function(done) {
    r.expr([10, 11, 12, 13, 14, 15, 12]).offsetsOf(12).run(connection).then(function(result) {
      assert.deepEqual(result, [2,6]);
      done();
    }).error(done);
  });

  it('offsetsOf - no result', function(done) {
    r.expr([10, 11, 12, 13, 14, 15, 12]).offsetsOf(1200).run(connection).then(function(result) {
      assert.deepEqual(result, []);
      done();
    }).error(done);
  });
  it('offsetsOf - function', function(done) {
    r.expr([10, 11, 12, 13, 14, 15, 12]).offsetsOf(r.row.eq(12)).run(connection).then(function(result) {
      assert.deepEqual(result, [2, 6]);
      done();
    }).error(done);
  });
