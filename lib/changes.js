...
/* Methods not overridden are:
  Changes.prototype.difference = 
  // Test the one below
  Changes.prototype.zip = 
  Changes.prototype.skip
  Changes.prototype.limit
...
Changes.throwTerminal = function(query) {
...
Changes.createListener = function(self) {
  return function(change) {
    ...
    if (sequence.length > 0) {
      sequence = util.toDatum(sequence);
      if (self.callbacks.length > 0) {
        var callback = self.callbacks.shift();
        callback(sequence, self.notes);
      }
      else {
