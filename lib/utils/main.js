...
util.revertDatum = function(value) {
  ...
  if (value === null) {
    ...
    if (ReqlDate === undefined) { ReqlDate = require(__dirname+"/../geo.js"); }
    ...
    if (ReqlGeometry === undefined) { ReqlGeometry = require(__dirname+"/../geo.js"); }
...
util.hasFields = function(obj, keys) {
  ...
  for(var i=0; i<keys.length; i++) {
    if (typeof keys[i] === 'string') {
      if ((obj.hasOwnProperty(keys[i]) === false) || (obj[keys[i]] == null)) {
        return false;
      }
    }
    else if (util.isPlainObject(keys[i])) {
