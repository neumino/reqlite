...
var GeographicLib = require("./../../external/geographiclib.js");
...
util.nonDeterministicOp = function() {
  throw new Error.ReqlRuntimeError('Could not prove argument deterministic.  Maybe you want to use the non_atomic flag?', []);
};
...
util.arityError = function(expected, found, query) {
  ...
  if (expected === 1) {
    throw new Constructor('Expected '+expected+' argument but found '+found+'.', query.frames);
  }
  else {
    throw new Constructor('Expected '+expected+' arguments but found '+found+'.', query.frames);
  }
};
