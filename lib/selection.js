...
Selection.prototype.filter = function(filter, options, query, internalOptions) {
  ...
  if (util.isFunction(filter)) {
    ...
    query.frames.push(1);
    return Promise.map(this.selection, function(context) {
...
Selection.prototype.update = function(rawUpdate, options, query) {
  return Promise.map(this.selection, function(doc) {
    return doc.update(rawUpdate, options, query)
...
Selection.prototype.orderBy = function(fields, options, query, internalOptions) {
  ...
  return Promise.map(this.selection, function(ref) {
    ...
    for(var i=0; i<resolved.length; i++) {
      console.log('++++++++++++++');
      console.log(resolved[i].original.doc)
      console.log(resolved[i].fields.sequence)
    ...
    for(var i=0; i<sequenceToSort.length; i++) {
      ...
      console.log(sequenceToSort.get(i).original.doc);
