...
//TODO toSequence should maybe not be called if we have a selection since we will
//lose the type
//TODO Add arity even for things that the JS driver cannot send
...
Query.prototype.evaluate = function(term, internalOptions) {
  ...
  if (Array.isArray(term) && (Array.isArray(term[1]))) {
    for(var i=0; i<term[1].length; i++) {
      if (util.isRawArgs(term[1][i])) {
        ...
        else if (term[1][i][1][0][0] === termTypes.MAKE_ARRAY) {
  ...
  switch(termType) {
    ...
    case termTypes.DIFFERENCE:
      ...
      sequence = util.toSequence(sequence, this);
    ...
    case termTypes.TO_ISO8601: // 99
      ...
      if (momentDate.milliseconds() === 0) {
        return momentDate.zone(momentDate.zone()).format('YYYY-MM-DDTHH:mm:ssZ');
      }
      else {
        return momentDate.zone(momentDate.zone()).format('YYYY-MM-DDTHH:mm:ss.SSSZ');
      }
    ...
    case termTypes.LITERAL:
      // r.literal can take 0 arguments
      this.frames.push(0);
      var value = this.evaluate(term[1][0], internalOptions)
      this.frames.pop();
