...
Query.prototype.changes = function(args, options, internalOptions) {
  ...
  util.assertArity(1, args, self);
  self.frames.push(0);
  return self.evaluate(args[0], internalOptions).then(function(tableSlice) {
    self.frames.pop();
    return new Changes(tableSlice, self);
  });
}
