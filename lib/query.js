...
Query.prototype.slice = function(args, options, internalOptions) {
  ...
  return self.evaluate(options, internalOptions).bind({}).then(function(options) {
    ...
    util.assertType(sequence, ["ARRAY", "PTYPE<BINARY>"], self);
    this.sequence = sequence;
    self.frames.pop();
    self.frames.push(1);
    return self.evaluate(args[1], internalOptions)
    ...
    if (args.length > 2) {
      ...
      return self.evaluate(args[2], internalOptions).bind(this).then(function(end) {
        ...
        self.frames.pop();
        return this.sequence.slice(this.start, end, this.options, self);
      });
    }
    else {
      return this.sequence.slice(this.start, undefined, this.options, self);
    }
  });
}
...
Query.prototype.skip = function(args, options, internalOptions) {
  ...
  return self.evaluate(args[0], internalOptions).then(function(sequenceOrBin) {
    util.assertType(sequenceOrBin, ['ARRAY', 'PTYPE<BINARY>'], self);
    this.sequenceOrBin = sequenceOrBin;
    self.frames.pop();
    self.frames.push(1);
    return self.evaluate(args[1], internalOptions)
  }).then(function(skip) {
    ...
    self.frames.pop();
    if (util.isBinary(this.sequenceOrBin)) {
      var buffer = new Buffer(this.sequenceOrBin.data, 'base64');
      buffer = buffer.slice(skip);
      this.sequenceOrBin.data = buffer.toString('base64');
      return this.sequenceOrBin;
    }
    else {
      return this.sequenceOrBin.skip(skip, self);
    }
  });
}
