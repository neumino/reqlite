...
Query.prototype.toEpochTime = function(args, options, internalOptions) {
  ...
  util.assertArity(1, args, self);
  self.frames.push(0);
  return self.evaluate(term[1][0], internalOptions).then(function(date) {
...
Query.prototype.time = function(args, options, internalOptions) {
  ...
  return self.evaluate(args[0], internalOptions).bind({}).then(function(year) {
    ...
    if (args.length === 4) {
      ...
      self.frames.pop();
      throw new Error.ReqlRuntimeError('Expected between 4 and 7 arguments but found '+args.length, self.frames);
    }
  }).then(function() {
...
Query.prototype.http = function(url) {
  self.frames.push(0);
  return self.evaluate(term[1][0], internalOptions).then(function(url) {
    self.frames.pop();
    return new Promise(function(resolve, reject) {
      var options = {
        url: url,
