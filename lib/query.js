...
Query.prototype.evaluate = function(term, internalOptions) {
  ...
  switch(termType) {
    ...
    case termTypes.JAVASCRIPT:
      ...
      if (constants.ENABLE_JS === true) {
        ...
        if ((result === Infinity) || (result === -Infinity)) {
          return Promise.reject(new Error.ReqlRuntimeError('Number return value is not finite', this.frames));
        }
      }
      else {
    ...
    case termTypes.IMPLICIT_VAR: // 13
      ...
      return Promise.resolve(this.context[Object.keys(this.context)[0]]);
    ...
    case termTypes.GET:
      return this.get(term[1], options, internalOptions);
    ...
    case termTypes.TIME: // 136
      ...
      //TODO sequentially evaluate
      return Promise.all([year, month, day, hours, minutes, seconds, milliseconds, timezone]).then(function(args) {
...
Query.prototype.eq = function(args, options, internalOptions) {
  ...
  self.frames.push(0);
  return self._eq(undefined, args, 0, internalOptions);
}

Query.prototype._eq = function(reference, left, index, internalOptions) {
  var self = this;
  if (index < left.length) {
    self.frames.push(index);
    return self.evaluate(left[index], internalOptions).then(function(value) {
      ...
      if (util.eq(reference, value) === false) {
        ...
        return self._eq(reference, left, index+1, internalOptions);
      }
    });
  }
  else {
...
Query.prototype.add = function(args, options, internalOptions) {
  ...
  self.frames.push(0);
  return self._add(undefined, args, 0, internalOptions);
}

Query.prototype._add = function(total, left, index, internalOptions) {
  var self = this;
  if (index < left.length) {
    self.frames.push(index);
    return self.evaluate(left[index], internalOptions).then(function(value) {
      ...
      if (total === undefined) {
        ...
        if (util.isDate(total)) {
          util.assertType(value, "NUMBER", self);
          total.epoch_time += value;
        }
        else if (util.isSequence(total)) {
          total = total.concat(value);
        }
        else {
          util.assertType(value, util.getType(total), self);
          total += value;
        }
      }
      return self._add(total, left, index+1, internalOptions);
    });
  }
  else {
    return total;
...
Query.prototype.merge = function(args, options, internalOptions) {
  ...
  return self.evaluate(args[0], internalOptions).then(function(result) {
    this.result = result;
    return Promise.reduce(args.slice(1), function(result, toMerge) {
      if (typeof result.merge === 'function') {
        return result.merge(toMerge, self);
      }
      else {
        return util.merge(result, toMerge, self, internalOptions);
      }
...
Query.prototype.bracket = function(args, options, internalOptions) {
  ...
  self.frames.push(0);
  return this.evaluate(args[0], internalOptions).bind({}).then(function(sequenceOrObject) {
    if (!util.isObject(sequenceOrObject) && !util.isSequence(sequenceOrObject)) {
    ...
    if (typeof key === 'number') {
      util.toSequence(this.sequenceOrObject, this);
    }
    return util.getBracket(this.sequenceOrObject, key, this);
  });
}
...
Query.prototype.update = function(args, options, internalOptions) {
  //function(selection, predicate, options, internalOptions) {
  var self = this;
  util.assertArity(2, args, self);
  self.frames.push(0);
  return self.evaluate(args[0], internalOptions).then(function(selection) {
    util.assertType(selection, 'SELECTION', self);
    self.frames.pop();
    return selection.update(args[1], options, self, internalOptions)
  });
}
...
Query.prototype.funcall = function(args, options, internalOptions) {
  ...
  var index = 0;
  return Promise.map(args, function(context) {
    index++;
    self.frames.pop();
    self.frames.push(index);
    util.assertPreDatum(context, self);
    return self.evaluate(context, internalOptions);
