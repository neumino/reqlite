  ...
  switch(termType) {
    ...
    case termTypes.IMPLICIT_VAR: // 13
      ...
      if (Object.keys(this.context).length > 1) {
        ...
        throw new Error.ReqlRuntimeError("Ambiguous implicit var:"+Object.keys(this.context).join(', '));
      }
...
Query.prototype.map = function(args, options, internalOptions) {
  ...
  return Promise.map(args.slice(0, -1), function(arg) {
    ...
    for(var i=0; i<sequences.length; i++) {
      util.toSequence(sequences[i], self);
    }
...
Query.prototype.count = function(args, options, internalOptions) {
  ...
  self.frames.push(0);
  return self.evaluate(args[0], internalOptions).then(function(sequenceOrBin) {
    self.frames.pop();
    if (util.isBinary(sequenceOrBin) && (args.length === 1)) {
    ...
    util.toSequence(sequenceOrBin, self);
    return sequenceOrBin.count(args[1], self, internalOptions);
  });
}
